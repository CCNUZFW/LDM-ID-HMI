...
def divide_data():
    ...
    # with open('data/train_set_20.json', 'w', encoding='utf8') as output_file:
    #     json.dump(train_set, output_file, indent=4, ensure_ascii=False)
    with open('data/val_set_20.json', 'w', encoding='utf8') as output_file:
...
def group_trainset_by_user():
    '''
    group logs in train_set by user to facilitate the computation of DOA
    :return:
    '''
    users = FastRBTree()
    with open('data/train_set.json', encoding='utf8') as i_f:
    ...
    for log in data:
        ...
        if user is None:
            ...
            user_log = [{'exer_id': log['exer_id'], 'knowledge_code': log['knowledge_code'],  'score': log['score']}]
            user['logs'] = user_log
            users.insert(user_id, user)
        else:
            user['logs'].append({'exer_id': log['exer_id'], 'knowledge_code': log['knowledge_code'],  'score': log['score']})
    json_format = []
    for user_id in users:
        json_format.append(users.get(user_id))

    with open('data/group_trainset_by_user.json', 'w', encoding='utf8') as o_f:
        json.dump(json_format, o_f, indent=4, ensure_ascii=False)

if __name__ == '__main__':
    divide_data()
    # group_trainset_by_exercise()
    # group_trainset_by_user()
